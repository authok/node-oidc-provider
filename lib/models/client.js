...
module.exports = function getClient(provider) {
  ...
  async function add(metadata, { ctx, store = false } = {}) {
    ...
    if (store) {
      await getAdapter().upsert(ctx, client.clientId, client.metadata());
  ...
  function remove(ctx, id) {
    return getAdapter().destroy(ctx, id);
  }
  ...
  class Client {
    ...
    static async find(ctx, id) {
      ...
      const properties = await getAdapter().find(ctx, id);

      if (!properties) {
        return undefined;
      }

      /* fix 让 adaptor 自行负责缓存
      ...
      */
      ...
      const client = await add(properties, { store: false });
