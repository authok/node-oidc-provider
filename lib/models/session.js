...
module.exports = (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async findByUid(ctx, uid) {
    const stored = await this.adapter.findByUid(ctx, uid);
    try {
      assert(stored);
  ...
  static async get(ctx) {
    ...
    if (cookieSessionId) {
      session = await this.find(ctx, cookieSessionId);
    }
  ...
  async save(ctx, ttl) {
    ...
    if (this.oldId) {
      await this.adapter.destroy(ctx, this.oldId);
    }

    const result = await super.save(ctx, ttl);
  ...
  async persist(ctx) {
    ...
    return this.save(ctx, this.exp - epochTime());
  }
