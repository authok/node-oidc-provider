...
module.exports = function getBaseToken(provider) {
  ...
  class Class {
    ...
    async save(ctx, ttl) {
      ...
      if (payload) {
        await this.adapter.upsert(ctx, this.jti, payload, ttl);
        this.emit('saved');
      } else {
        this.emit('issued');
      }
    ...
    async destroy(ctx) {
      await this.adapter.destroy(ctx, this.jti);
      this.emit('destroyed');
    }
    ...
    static async find(ctx, value, { ignoreExpiration = false } = {}) {
      if (typeof value !== 'string') {
        return undefined;
      }

      const stored = await this.adapter.find(ctx, value);
