...
module.exports = async function resumeAction(allowLisst, resumeRouteName, ctx, next) {
  ...
  const interactionSession = await ctx.oidc.provider.Interaction.find(ctx, cookieId);
  ...
  if (
    ...
    if (interactionSession.session && interactionSession.session.uid) {
      ...
      await interactionSession.save(ctx, interactionSession.exp - epochTime());
    }
  ...
  await interactionSession.destroy(ctx);
