...
module.exports = function authorizationAction(provider, endpoint) {
  const {
    features: { claimsParameter, resourceIndicators, webMessageResponseMode },
    extraParams,
  ...
  if (resourceIndicators.enabled) {
    allowList.add('audience');
    rejectDupesMiddleware = rejectDupes.bind(undefined, { except: new Set(['audience']) });
  }
  ...
  use(() => processRequestObject.bind(undefined, allowList, rejectDupesMiddleware), A, DA, PAR, BA);
