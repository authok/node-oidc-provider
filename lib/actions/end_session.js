...
module.exports = {
  init: [
    ...
    async function endSessionChecks(ctx, next) {
      ...
      if (params.id_token_hint) {
        ...
        client = await ctx.oidc.provider.Client.find(ctx, clientId);
        if (!client) {
        ...
        client = await ctx.oidc.provider.Client.find(ctx, params.client_id);
        if (!client) {
  ...
  confirm: [
    ...
    async function endSession(ctx, next) {
      ...
      if (backchannelLogout.enabled) {
        ...
        for (const clientId of clientIds) { // eslint-disable-line no-restricted-syntax
          if (params.logout || clientId === state.clientId) {
            const client = await ctx.oidc.provider.Client.find(ctx, clientId); // eslint-disable-line no-await-in-loop, max-len
            if (client) {
      ...
      if (state.clientId) {
        ctx.oidc.entity('Client', await ctx.oidc.provider.Client.find(ctx, state.clientId));
      }
  ...
  success: [
    ...
    async function postLogoutSuccess(ctx) {
      if (ctx.oidc.params.client_id) {
        const client = await ctx.oidc.provider.Client.find(ctx, ctx.oidc.params.client_id);
        if (!client) {
          throw new InvalidClient('client is invalid', 'client not found');
        }
        ctx.oidc.entity('Client', client);
