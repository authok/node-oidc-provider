...
module.exports.handler = async function clientCredentialsHandler(ctx, next) {
  ...
  const resourceServers = ctx.oidc.resourceServers.values;
  if (resourceServers) {
    if (ctx.oidc.resourceServers.values.length > 1) {
    ...
    let clientGrantScopes = (await getClientGrantScopes(ctx, client, resourceServer.audience)) || [];
