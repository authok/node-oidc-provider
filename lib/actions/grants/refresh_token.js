...
module.exports.handler = async function refreshTokenHandler(ctx, next) {
  ...
  let refreshToken = await RefreshToken.find(ctx, refreshTokenValue, { ignoreExpiration: true });
  ...
  const grant = await ctx.oidc.provider.Grant.find(ctx, refreshToken.grantId, {
  ...
  if (
    ...
    await refreshToken.consume(ctx);
    ...
    ctx.oidc.entity('RefreshToken', refreshToken);
    refreshTokenValue = await refreshToken.save(ctx);
  }
  ...
  if (resource) {
    console.log('从 refreshToken 找到resource: ', resource);
