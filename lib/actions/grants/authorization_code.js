...
module.exports.handler = async function authorizationCodeHandler(ctx, next) {
  ...
  const code = await ctx.oidc.provider.AuthorizationCode.find(ctx, ctx.oidc.params.code, {
  ...
  const grant = await ctx.oidc.provider.Grant.find(ctx, code.grantId, {
    ignoreExpiration: true,
  });
  ...
  if (dPoP) {
    const unique = await ReplayDetection.unique(
      ctx, ctx.oidc.client.clientId, dPoP.jti, dPoP.iat + iatTolerance,
    );
  ...
  const accessToken = await at.save(ctx);
  ...
  if (await issueRefreshToken(ctx, ctx.oidc.client, code)) {
    ...
    refreshToken = await rt.save(ctx);
  }

  let idToken;
  if (code.scopes.has('openid')) {
    // 没有开启 features.claimsParameter.enabled 参数的话, 这里 claims是空, 从而 claims 为空
...
module.exports.parameters = new Set(['code', 'redirect_uri', 'organization']);
