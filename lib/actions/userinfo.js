...
module.exports = [
  ...
  async function validateAccessToken(ctx, next) {
    ...
    const accessToken = await ctx.oidc.provider.AccessToken.find(ctx, accessTokenValue);
    ...
    if (dPoP) {
      const unique = await ctx.oidc.provider.ReplayDetection.unique(ctx,
  ...
  async function loadClient(ctx, next) {
    const client = await ctx.oidc.provider.Client.find(ctx, ctx.oidc.accessToken.clientId);
  ...
  async function loadGrant(ctx, next) {
    const grant = await ctx.oidc.provider.Grant.find(ctx, ctx.oidc.accessToken.grantId, {
