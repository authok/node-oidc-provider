...
async function getInteraction(req, res) {
  ...
  const interaction = await this.Interaction.find(ctx, id);
  if (!interaction) {
    throw new SessionNotFound('interaction session not found');
  }

  if (interaction.session && interaction.session.uid) {
    const session = await this.Session.findByUid(ctx, interaction.session.uid);
    if (!session) {
...
class Provider extends events.EventEmitter {
  ...
  async interactionResult(req, res, result, { mergeWithLastSubmission = true } = {}) {
    ...
    await interaction.save(ctx, interaction.exp - epochTime());
  ...
  async backchannelResult(ctx, request, result, {
    acr,
    amr,
    authTime,
    sessionUid,
    ...
    console.log('backchannelResult');
    if (typeof request === 'string' && request) {
      // eslint-disable-next-line no-param-reassign
      request = await this.#BackchannelAuthenticationRequest.find(ctx, request, {
    ...
    const client = await this.#Client.find(ctx, request.clientId);
    if (!client) {
    ...
    if (typeof result === 'string' && result) {
      // eslint-disable-next-line no-param-reassign
      result = await this.#Grant.find(ctx, result);
      if (!result) {
