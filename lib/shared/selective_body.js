...
async function selectiveBody(_cty, ctx, next) {
  if (true || ctx.is(_cty)) {
    console.log('ctx is cty', _cty);
    const cty = ctx.get('content-type') || _cty;
...
is not recommended, resolving to use req.body or request.body instead');
    ...
    throw new InvalidRequest(`only ${_cty} content-type bodies are supported on ${ctx.method} ${ctx.path}`);
  } else {
    ctx.oidc.body = {};
    await next();
  }
